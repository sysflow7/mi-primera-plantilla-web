export default {
  async fetch(request, env) {
    ...
    const isSidenWorkerPreview = host.endsWith(".workers.dev") && host.includes("siden-corporativo");

    const slugify = (value) => String(value || "").toLowerCase().trim()
      .replace(/[^a-z0-9-]/g, "-").replace(/-+/g, "-").replace(/^-|-$/g, "");

    let instanceId = "";
    if (baseHost || isSidenWorkerPreview) {
    ...
    if (!esAreaServicio && direccion.latitud !== "" && direccion.latitud !== undefined &&
      ...
      datosNegocio.geo = {
        "@type": "GeoCoordinates", latitude: Number(direccion.latitud), longitude: Number(direccion.longitud)
      };
    }
    ...
    const ciudadVisible = esAreaServicio
      ? (Array.isArray(negocio.areasServicio) && negocio.areasServicio.length
        ? negocio.areasServicio.map(area => typeof area === "string" ? area : area?.nombre).filter(Boolean).join(", ")
        : negocio.pais || "Área de servicio") : ciudad;
