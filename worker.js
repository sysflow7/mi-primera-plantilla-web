export default {
    async fetch(request, env) {
        ...
        const baseHost = host.replace(/^www\./, "") === baseDomain;
        ...
        if (baseHost) {
            ...
            instanceId = slugify(await env.SIDEN_REGISTRY.get(host));
        }

        if (!instanceId) {
            return new Response("Sitio SIDeN no configurado.", {
                status: 404,
        ...
        const esRutaPagina = url.pathname === "/" ||
            rutasMultipagina.includes(rutaNormalizada(url.pathname));
        ...
        if (url.pathname === "/sitemap.xml") {
            ...
            const contenido = '<?xml version="1.0" encoding="UTF-8"?>' +
                ...
                rutas.map(ruta => `<url><loc>${url.origin}${ruta}</loc></url>`).join("") +
                '</urlset>';
            return new Response(contenido, {
                headers: {
        ...
        const paginaActual = negocio.modoSitio === "multi" && Array.isArray(negocio.paginas)
            ? negocio.paginas.find(pagina =>
                rutaNormalizada(pagina.ruta) === rutaNormalizada(url.pathname)) : null;

        const tituloSEO = paginaActual?.tituloSEO || negocio.tituloSEO || negocio.seo?.titulo ||
            (paginaActual?.nombre ? `${paginaActual.nombre} | ${negocio.nombre}` :
                `${negocio.nombre} | ${negocio.ciudad}`);

        const descripcionSEO = paginaActual?.descripcionSEO || negocio.descripcionSEO ||
            negocio.seo?.descripcion || paginaActual?.descripcion || negocio.descripcion ||
            negocio.slogan || negocio.nombre;
        ...
        const schemaTypes = {
            comercio: "Store", hardwarestore: "HardwareStore", tienda: "Store",
            profesional: "ProfessionalService", abogado: "LegalService", medico: "Physician",
            médico: "Physician", contador: "ProfessionalService", arquitecto: "ProfessionalService",
            electricista: "Electrician", fotografo: "ProfessionalService", fotógrafo: "ProfessionalService",
            consultor: "ProfessionalService", psicologo: "Psychologist", psicólogo: "Psychologist",
            dentista: "Dentist", restaurante: "Restaurant", restaurant: "Restaurant"
        };
        const tipoClave = String(paginaActual?.tipoNegocio || negocio.tipoNegocio || "LocalBusiness").toLowerCase();
        const datosNegocio = {
            "@context": "https://schema.org", "@type": schemaTypes[tipoClave] || "LocalBusiness",
            "@id": negocioId, name: negocio.nombre, description: descripcionSEO,
            url: canonical, telephone: negocio.telefono
        };
        ...
        if (Object.keys(postalAddress).length > 0 && !esAreaServicio) {
            datosNegocio.address = { "@type": "PostalAddress", ...postalAddress };
        }
        ...
        const redes = [negocio.facebook, negocio.instagram]
            .filter(red => typeof red === "string" && red.startsWith("http"));
        if (redes.length) datosNegocio.sameAs = redes;

        if (!esAreaServicio && direccion.latitud !== "" && direccion.latitud !== undefined &&
            direccion.longitud !== "" && direccion.longitud !== undefined) {
            datosNegocio.geo = {
                "@type": "GeoCoordinates", latitude: Number(direccion.latitud),
                longitude: Number(direccion.longitud)
            };
        }

        if (Array.isArray(negocio.areasServicio) && negocio.areasServicio.length) {
            const areas = negocio.areasServicio.map(area => {
        ...
        if (Array.isArray(negocio.horarios) && negocio.horarios.length) {
            datosNegocio.openingHoursSpecification = negocio.horarios.flatMap(horario =>
                (horario.dias || []).map(dia => ({
                    "@type": "OpeningHoursSpecification", dayOfWeek: dia,
                    opens: horario.abre, closes: horario.cierra
                }))
            );
        }
        ...
        const escHtml = valor => String(valor ?? "")
            ...
            .replace(/"/g, "&quot;").replace(/'/g, "&#39;");
        const escJson = valor => JSON.stringify(valor)
            .replace(/</g, "\\u003c").replace(/>/g, "\\u003e").replace(/&/g, "\\u0026")
            .replace(/\u2028/g, "\\u2028").replace(/\u2029/g, "\\u2029");

        const ciudad = direccion.ciudad || negocio.ciudad || "";
        const ciudadVisible = esAreaServicio
            ? (Array.isArray(negocio.areasServicio) && negocio.areasServicio.length
                ? negocio.areasServicio.map(area => typeof area === "string" ? area : area?.nombre)
                    .filter(Boolean).join(", ")
                : negocio.pais || "Área de servicio")
            : ciudad;
        const direccionTexto = esAreaServicio ? "" :
            (negocio.direccionTexto || direccion.calle || ciudad || "");
        const tituloUbicacion = negocio.etiquetas?.ubicacion ||
        ...
        const reemplazos = {
            ...
            "__BUSINESS_NAME__": escHtml(negocio.nombre), "__BUSINESS_TYPE__": escHtml(negocio.etiquetaTipo || ""),
            "__H1_TITLE__": escHtml(h1Title), "__H1_DESCRIPTION__": escHtml(h1Description),
            "__BUSINESS_DESCRIPTION__": escHtml(negocio.descripcion || descripcionSEO),
            "__LOCATION_TITLE__": escHtml(tituloUbicacion), "__CITY__": escHtml(ciudadVisible),
            "__ADDRESS__": escHtml(direccionTexto), "__PHONE__": escHtml(negocio.telefono || ""),
        ...
        const runtimeConfig = {
            ...negocio,
            siden: {
                ...(negocio.siden || {}), instanceId: negocio.siden?.instanceId || instanceId,
                host, canonicalOrigin: url.origin, assetPrefix: sitePrefix
            }
        };
